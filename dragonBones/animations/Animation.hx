 ...
 * @see dragonBones.animations.AnimationState
...
@:allow(dragonBones) class Animation extends BaseObject
{
	...
	private function _advanceTime(passedTime:Float):Void
	{
		...
		if (_armature.inheritAnimation && _armature._parent != null) // Inherit parent animations timeScale.
		{
	...
	public function playConfig(animationConfig:AnimationConfig):AnimationState 
	{
		...
		// Child armature play same name animations.
		var slots:Vector<Slot> = _armature.getSlots();
		...
		if (animationConfig.fadeInTime <= 0.0) // Blend animations state, update armature.
		{
